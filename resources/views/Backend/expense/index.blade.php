    ...
    <div class="pd-20 card-box mb-30">
        <div class="clearfix">
            <div class="my-2 pull-left">
                <h4 class="text-blue h4">Expense Section</h4>
            </div>
            <div class="pull-right">
                <a href="{{ route('admin.expense.create') }}" class="btn btn-primary btn-sm scroll-click"><i
                ...
                <tbody>
                    @foreach ($expenses as $value)
                        <tr>
                            ...
                            <td>{{ $value->expense_reason }}</td>
                            <td>৳ {{ $value->amount }}</td>
                            <td>{{ $value->date }}</td>
                            <td><span class="p-1 text-white rounded span bg-info">{{ $value->user->name }}</span></td>
                            <td>
                                @if ($value->status == 1)
                            ...
                            <td>
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu dropdown-menu-right dropdown-menu-icon-list">
                                        ...
                                        <a class="dropdown-item" href="{{ route('admin.expense.edit', $value->id) }}"><i
                                                class="dw dw-edit2"></i> Edit</a>
                                        <form action="{{ route('admin.expense.destroy', $value->id) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
